    ...
    >
      <div className="flex items-center justify-between">
        <div className="flex-1 text-right">
          <p className="text-sm md:text-base text-gray-600 mb-2 arabic-text">{title}</p>
          <p className="text-2xl md:text-3xl font-bold text-blue-600">{value}</p>
          {trend && (
            <p className="text-xs md:text-sm text-gray-500 mt-1 arabic-text">{trend}</p>
          )}
        </div>
        <div className={`p-3 bg-blue-50 rounded-2xl ${iconColor} ms-4`}>
